...
public class CreateConsultaValidator : AbstractValidator<ConsultaRequest>
{
	...
	{
		RuleFor(x => x.MedicoId).NotEmpty().NotNull().WithMessage("Médico deve ser informado");
		RuleFor(x => x.PacienteId).NotEmpty().NotNull().WithMessage("Paciente deve ser informado");
